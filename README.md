...
공식 데이터 자동화는 평일 18:15·18:45·20:30 KST에 순서대로 시도한다. GitHub 예약 실행이 지연되더라도 실제 시작 시각이 아니라 가장 최근의 의도된 cron 슬롯을 KST로 복원해 목표 거래일을 정하므로, 금요일 작업이 토요일 UTC에 늦게 시작돼도 금요일 세션을 요청한다. 18:15·18:45에는 KRX의 목표일 공식 세션이 아직 없으면 공개 파일을 전혀 바꾸지 않고 다음 실행으로 넘긴다. 첫 성공 뒤 같은 날짜의 후속 실행은 공급자 재수집과 파생 데이터 커밋을 생략하되, 현재 checkout을 다시 검증·빌드·배포하고 공개 파일의 정확한 바이트 일치를 확인해 Pages 누락을 자가복구한다. 20:30에는 Open API와 인증 KRX 목표일 세션을 함께 확인해 휴장일이면 역시 아무 파일도 바꾸지 않고 종료하며, Open API만 늦고 인증 KRX에 목표일 세션이 있으면 인증 경로로 확정 분석을 계속한다. 실제 거래일인데 최종 시도까지 실패하면 마지막 정상 시장 결과를 보존하고 `stale` 또는 `degraded` 운영 상태를 발행한 뒤 Actions 실행도 실패로 종료해 운영자가 놓치지 않게 한다. 최종 성공은 수집 job 전체, 배포, 공개 파일 SHA-256 검증이 모두 성공하고 공개 `summary.json`의 `dataAsOf`·`expectedDataAsOf`·공급자 최신성 판정이 확인된 거래일과 일치해야 완료로 인정한다. 요청일보다 이전 거래일은 공급자가 휴장일임을 확인한 no-op receipt가 있을 때만 허용한다. 수집 성공 뒤 테스트나 배포에서 중단된 실행은 완료로 판단하지 않는다.

ETF 교차검증이 실패하면 해당 비교는 사유와 함께 `unavailable`로 표시하고 수익률·거래·주문은 노출하지 않는다. 이 안전한 차단 상태는 전체 갱신을 막는 테스트 오류와 구분해 검증한다. 정상 교차검증에도 설명 없이 계산이 실패하거나 산출 불가 상태에서 유효한 결과를 노출하면 계속 배포를 차단한다. 수집 receipt의 `diagnostics`에는 공급자 응답 원문 대신 ETF별 검증 상태와 실패 사유 코드만 기록한다. 화면 테스트의 잠정 신호 시각은 고정하고 실행 제한 시계와 분리한다.
...
Mac이 잠들어 있거나 로그아웃된 경우 15:47 잠정 신호는 보장되지 않는다. 공식 로컬 fallback의 `caffeinate -i`는 유휴 수면만 억제하므로, 배터리 유지보수 수면·전원 종료·로그아웃 중 수집을 보장하지 않는다. 절대 종료시각이 지난 작업은 재개 직후 watchdog이 중단한다. live 실행 상태는 별도 schema v2 `live-signal-status.json`에 기록한다. 16:00 이후 깨어난 잠정 실행은 만료된 신규 신호를 만들지 않으며, 다음 공식 갱신과 Pages 빌드도 기존 공개 잠정 신호의 만료를 방어적으로 적용한다. GitHub Secrets를 직접 등록하면 동일한 공식 18:15·18:45·20:30 워크플로가 GitHub-hosted runner에서도 독립적으로 동작한다. 잠정 신호의 수동 Actions 검증도 세 secret이 필요하며 캡처 창 밖에서는 성공적인 no-op으로 끝난다. `data/live-signal.json`은 당일 입력과 기본 3개 모형만 담으며 확정 `history.json`, 사건 연구, 차트 성과, 백테스트에는 들어가지 않는다.
