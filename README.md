...
첫 화면은 20일 비중첩 사건 연구와 226490 롱/현금 기본 모형, 롱/숏/현금 합성 비교를 함께 읽어 근거 우선 결론을 만든다. 서버가 검증해 발행하는 기본 청산선은 롱 80·숏 20 백분위다. 사용자가 화면에서 롱 청산선을 50~94로 바꾸면 브라우저는 서버가 발행한 일별 신호와 조정가격만으로 거래 경로를 다시 계산하며, 회귀·상태·사건 표본을 다시 적합하거나 임계값을 최적화하지 않는다.

롱/숏/현금 비교의 숏은 진입비용 차감 후 가용자본 1배의 고정 수량 합성 숏이다. 조정 총수익 가격으로 배당의 경제효과를 반영하지만 대차 가능성·대차료·리콜·증거금·강제청산·공매도 주문 규칙·시장충격을 모형화하지 않는다. 따라서 실제 실행성과나 역ETF 성과로 해석할 수 없으며 연구용 비교에 한정한다.
...
- `data/strategy-comparison.json`: 기본 80 롱/숏 정책, 청산선 민감도와 동적 시나리오 계약
