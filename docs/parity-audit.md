# Fear & Greed 계산 경계와 Control API 연결 구조
...
Backend 연결과 parity gate를 **구현 완료**했다. 구현 완료의 의미는
JavaScript와 Python에 같은 통계 알고리즘을 두 벌 유지한다는 뜻이 아니다.
분석 결과의 유일한 권위를 Python으로 정한다. 브라우저 계산은 항상
`preview_only_not_result_authority`이며, 공개 artifact의 요청·설정·데이터·
코드 identity와 SHA-256·byte 크기를 모두 검증한 exact 입력에만
`python_control_api_verified` 권위를 부여한다.
...
- 사건 신뢰구간과 초과수익은 브라우저에서 정식 결과로 승격하지 않는다.
  ...
  서버 결과 권위로 사용한다.
- 화면의 16개 복합 입력은 17개 wire field로 직렬화되며
  `fear-greed/control-inputs-v1`에서 `requested`,
...
하나라도 어긋나면 종료 코드가 0이 아니므로 controlled 분석과 배포를 중단한다.
...
6. Owner token은 브라우저 메모리에만 두고 API base만 선택적으로 저장한다.
7. `.github/workflows/controlled-analysis.yml`은 기존 정기 갱신과 별도로
   불변 artifact 하나만 커밋하며, Pages 공개 byte가 일치한 뒤에만 성공
   callback을 보낸다.
